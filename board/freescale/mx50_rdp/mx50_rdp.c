...
//	puts("Check battery\n");
	...
	volt_recovery = volt_bat;				/* Add by Joe for recovery voltage. */	
...
pmic_reg(slave, 0, val, 1);
val = pmic_reg(slave, 3, 0, 0);
pmic_reg(slave, 3, val, 1);
			udelay(10000000);
...
//	printf("led on: %x\n",val);
	...
	printf("Battery voltage:%dmV\tcapacity:%d%\n", volt_bat, soc);
	...
	if( soc < 4 || volt_bat < 3640 )
	{
		...
		{
			printf("Battery discharge...\n");
			//no charger, shut down
			puts("Low battery without charger, device emergency shut down.\n");
			udelay(1000);
			val = pmic_reg(slave, MC13892_REG_POWER_CTL0, 0, 0);
			val |= 0x8;
			pmic_reg(slave,  MC13892_REG_POWER_CTL0, val, 1);
...
int esdhc_gpio_init(bd_t *bis)
{
	...
	for (index = 0; index < CONFIG_SYS_FSL_ESDHC_NUM;
		...
		case 0:
			...
			mxc_request_iomux(MX50_PIN_DISP_D8, IOMUX_CONFIG_ALT1);//gp2_8 pin for SD_EN
